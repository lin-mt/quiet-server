...
import com.github.quiet.repository.scrum.ScrumIterationRepository;
...
public class ScrumIterationManager {
  ...
  private void endVersion(Long iterationId, Long versionId) {
    ...
    List<ScrumVersion> unfinishedVersions =
        versionService.listAllChildren(versionId).stream()
            .filter(v -> Objects.isNull(v.getEndTime()))
            .collect(Collectors.toList());
    if (CollectionUtils.isEmpty(unfinishedIterations)
        && CollectionUtils.isEmpty(unfinishedVersions)) {
