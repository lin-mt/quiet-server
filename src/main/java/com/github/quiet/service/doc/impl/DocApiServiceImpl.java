...
import static com.github.quiet.entity.doc.QDocApi.docApi;
...
public class DocApiServiceImpl implements DocApiService {
  ...
  private void checkInfo(DocApi api) {
    DocApi exist =
        repository.findByProjectIdAndPathAndMethod(
            api.getProjectId(), api.getPath(), api.getMethod());
    if (exist != null && !exist.getId().equals(api.getId())) {
      throw new ServiceException(
          "api.path.method.exist", api.getProjectId(), api.getPath(), api.getMethod());
    }
  }
