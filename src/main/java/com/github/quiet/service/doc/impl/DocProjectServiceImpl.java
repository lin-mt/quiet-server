...
import static com.github.quiet.entity.doc.QDocProject.docProject;
...
public class DocProjectServiceImpl implements DocProjectService {
  ...
  public DocProject saveOrUpdate(@NotNull DocProject entity) {
    ...
    if (groupId == null || groupId <= 0) {
      builder.and(docProject.creator.eq(UserUtil.getId())).and(docProject.groupId.isNull());
    } else {
    ...
    if (groupId == null || groupId <= 0) {
      BooleanBuilder where =
          SelectBooleanBuilder.booleanBuilder()
              .and(docProject.creator.eq(UserUtil.getId()))
