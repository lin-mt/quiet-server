...
public class MessageSourceUtil {
  ...
  public static String getMessage(
    ...
    try {
      List<Locale.LanguageRange> languageRanges =
          StringUtils.isNoneBlank(acceptLanguage)
              ? Locale.LanguageRange.parse(acceptLanguage)
              : Collections.emptyList();
      locales =
          languageRanges.stream()
              .map(range -> Locale.forLanguageTag(range.getRange()))
              .filter(locale -> StringUtils.isNoneBlank(locale.getDisplayName()))
  ...
  private static String getMessageFromMessageSource(
      MessageSource messageSource, Locale locale, String code, Object[] param) {
    String message;
    try {
      message = messageSource.getMessage(code, param, locale);
    } catch (NoSuchMessageException e) {
      message =
          messageSource.getMessage(MessageSourceCode.UNKNOWN_CODE, new Object[] {code}, locale);
    }
    return message;
  }
